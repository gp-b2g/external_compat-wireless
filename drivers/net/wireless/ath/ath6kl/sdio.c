...
extern void *ath6kl_fetch_memory(void);
			     ...
			     const struct sdio_device_id *id)
{
...
#ifdef ATH6KL_USES_PREALLOCATE_MEM
	pr_info("ath6kl use preallocated memory for dma buffer\n");
	ar_sdio->dma_buffer = ath6kl_fetch_memory();
#else
	...
	if (ret) {
        ...
        if (wlan_properties_kobj) {
                ...
                ath6kl_err("Fail to create and add android_wlan kobject\n");
		ret = -ENOMEM;
		goto err_core_alloc;
...
err_dma:
#ifndef ATH6KL_USES_PREALLOCATE_MEM

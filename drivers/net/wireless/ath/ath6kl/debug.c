...
#ifdef SUPP_ACL_4_CUST
void _dbg_dump_mac_cmd(struct wmi_ap_acl_mac_cmd *cmd)
{
	printk("\n\n"
               "cmd->action   = %d\n"
               "cmd->index    = %d\n"
               "cmd->wildcard = %d\n"
               "cmd->mac      = %02x:%02x:%02x:%02x:%02x:%02x\n\n\n",
               cmd->action, cmd->index, cmd->wildcard,
...
int _wmic_ether_aton_wild(const char *orig, u8 *eth, u8 *wild)
{
  const char *bufp;
  unsigned char val, c;
  int i=0;
...
u8 _mac_cmp_wild(u8 *mac, u8 *new_mac, u8 wild, u8 new_wild)
{
    u8 i;
...
u8 _acl_add_del_mac(struct wmi_ap_acl *a, struct wmi_ap_acl_mac_cmd *acl)
{
    ...
    {
        if(_mac_cmp_wild(a->acl_mac[i], acl->mac, a->wildcard[i], acl->wildcard)==0)
            already_avail = i;
    ...
    else if(acl->action == DEL_MAC_ADDR)
    {
        ...
        memset(a->acl_mac[acl->index], 0, ATH_MAC_LEN);
...
struct ath6kl_vif *_ath6kl_vif_softap(struct ath6kl *ar)
{
	struct ath6kl_vif *vif, *tmp;
	bool vif_softap = false;
	...
	list_for_each_entry_safe(vif, tmp, &ar->vif_list, list) {
...
static ssize_t ath6kl_acl_policy_read(struct file *file,
				char __user *user_buf,
				size_t count, loff_t *ppos)
{
	struct ath6kl *ar = file->private_data;
    ...
    len += snprintf(buf + len, 2048 - len, "%d\n", ar->ap_acl.policy & ~AP_ACL_RETAIN_LIST_MASK);
...
static ssize_t ath6kl_acl_policy_write(struct file *file,
				const char __user *user_buf,
				size_t count, loff_t *ppos)
{
	struct ath6kl *ar = file->private_data;
    ...
    vif = _ath6kl_vif_softap(ar);
    if(vif == NULL)
    {
        ath6kl_err("[%s]  ACL only supported in softap!\n", __FUNCTION__);
		return -EIO;
    }

    
	if (copy_from_user(&policy, user_buf, sizeof(policy)))
		return -EFAULT;

	policy = atoi((char *) &policy);
        printk("[%s]    policy = %d!\n", __FUNCTION__, policy);

    printk("[%s]    policy = %d!", __FUNCTION__, policy);




    if(policy == ar->ap_acl.policy)
    {
        /* No change in policy */
        return count;
    }
    ...
    ret = wmi_ap_set_acl_policy_cmd(ar->wmi, vif->fw_vif_idx, policy);
    if(!ret)
    {
        if(!(policy & AP_ACL_RETAIN_LIST_MASK)) {
            /* clear ACL list */
            memset(&ar->ap_acl, 0, sizeof(struct wmi_ap_acl));
        }            
        ar->ap_acl.policy = policy;
	printk("[%s] change policy success.\n", __func__);
        return count;
    }
    else
    {
...
/* debug fs for acl_policy. */
static const struct file_operations fops_acl_policy = {
	.read = ath6kl_acl_policy_read,
	.write = ath6kl_acl_policy_write,
	.open = ath6kl_debugfs_open,
	.owner = THIS_MODULE,
	.llseek = default_llseek,
};
...
static inline int is_mac_null(u8 *mac)
{
...
void print_wild_mac(u8 *buf, unsigned int *len, unsigned char *mac, char wildcard)
{
    int i;
    ...
    *len += snprintf(buf + *len, 2048 - *len, "    ");
    ...
    for(i=0;i<5;i++) {
        if(wildcard & (1<<i))   
            ...
            *len += snprintf(buf + *len, 2048 - *len, "*:");
            ...
            //printf("%02X:", mac[i]);
            *len += snprintf(buf + *len, 2048 - *len, "%02X:", mac[i]);
    ...
    if(wildcard & (1<<i))
        ...
        *len += snprintf(buf + *len, 2048 - *len, "*\n");
        ...
        //printf("%02X\n", mac[5]);
        *len += snprintf(buf + *len, 2048 - *len, "%02X\n", mac[5]);
}


#define BUF_SIZE   1000

static ssize_t ath6kl_acl_mac_list_read(struct file *file,
				char __user *user_buf,
				size_t count, loff_t *ppos)
{
	struct ath6kl *ar = file->private_data;
	...
	unsigned int len = 0;
        ...
        for(i=0;i<AP_ACL_SIZE;i++) {
            if(!is_mac_null(ar->ap_acl.acl_mac[i]) || ar->ap_acl.wildcard[i]) {
                len += snprintf(buf + len, BUF_SIZE - len, "%d ",i);
                print_wild_mac(buf,&len, ar->ap_acl.acl_mac[i], ar->ap_acl.wildcard[i]);
            }
        }
...
/* debug fs for acl_mac_list. */
static const struct file_operations fops_acl_mac_list = {
	.read = ath6kl_acl_mac_list_read,
	.open = ath6kl_debugfs_open,
	.owner = THIS_MODULE,
	.llseek = default_llseek,
};
...
static ssize_t ath6kl_acl_mac_add_write(struct file *file,
				const char __user *user_buf,
				size_t count, loff_t *ppos)
{
	struct ath6kl *ar = file->private_data;
    ...
    struct wmi_ap_acl_mac_cmd   acl_mac_cmd;
	char buf[32];
	unsigned int len;
        int ret;
    ...
    printk("[%s]    cmd = %s!", __FUNCTION__, buf);


    acl_mac_cmd.action = ADD_MAC_ADDR;
    if(_wmic_ether_aton_wild(buf, acl_mac_cmd.mac, &acl_mac_cmd.wildcard) != 0) {
        ath6kl_err("[%s]  bad mac address!\n", __FUNCTION__);
		return -EINVAL;;
    }


    if(_acl_add_del_mac(&ar->ap_acl, &acl_mac_cmd)) {
        _dbg_dump_mac_cmd(&acl_mac_cmd);
        ret = wmi_ap_acl_mac_list_cmd(ar->wmi, vif->fw_vif_idx, &acl_mac_cmd);
        if (ret) {
            ath6kl_err("[%s] wmi access error!\n", __func__);
            return -EIO;
        }
    } else {
        ath6kl_err("[%s]  ACL list error!\n", __FUNCTION__);
        return -EIO;
    }
...
/* debug fs for acl_mac_add. */
static const struct file_operations fops_acl_mac_add = {
	.write = ath6kl_acl_mac_add_write,
	.open = ath6kl_debugfs_open,
	.owner = THIS_MODULE,
	.llseek = default_llseek,
};
...
static ssize_t ath6kl_acl_mac_del_write(struct file *file,
				const char __user *user_buf,
				size_t count, loff_t *ppos)
{
	struct ath6kl *ar = file->private_data;
    ...
    printk("[%s]    cmd = %s len = %d isdigit buf[0] %d!\n", __FUNCTION__, buf, strlen(buf), ISDIGIT(buf[0]));


    memset((char *)&acl_mac_cmd, 0 , sizeof(struct wmi_ap_acl_mac_cmd));
    acl_mac_cmd.action = DEL_MAC_ADDR;
#if 0
    //Knivo
    if(_wmic_ether_aton_wild(buf, acl_mac_cmd.mac, &acl_mac_cmd.wildcard) != 0) {
        ath6kl_err("[%s]  bad mac address!\n", __FUNCTION__);
    ...
    if( (strlen(buf) == 3) && ISDIGIT(buf[0])  && ISDIGIT(buf[1]) ) {
        acl_mac_cmd.index = simple_strtol(buf, NULL, 0);
    } else if( (strlen(buf) == 2) && _isdigit(buf[0]) ) {
        acl_mac_cmd.index = simple_strtol(buf, NULL, 0);
    } else {
        ath6kl_err("[%s]  bad ACL index!\n", __FUNCTION__);
		return -EINVAL;
    }

    // check index here to avoid bad ACL index goto fw!
    if (acl_mac_cmd.index > AP_ACL_SIZE) {
	ath6kl_err("[%s] bad ACL index! input index = %d\n", __FUNCTION__, acl_mac_cmd.index);
	return -EINVAL;
    }

    if (!(ar->ap_acl.index & (1 << acl_mac_cmd.index))) {
	ath6kl_err("[%s] bad ACL index! input index = %d\n", __FUNCTION__, acl_mac_cmd.index);
	return -EINVAL;
    }
    ...
    _dbg_dump_mac_cmd(&acl_mac_cmd);
    if(!wmi_ap_acl_mac_list_cmd(ar->wmi, vif->fw_vif_idx, &acl_mac_cmd))
    {
        if(_acl_add_del_mac(&ar->ap_acl, &acl_mac_cmd)) {
            return count;
        } else {
...
/* debug fs for acl_mac_del. */
static const struct file_operations fops_acl_mac_del = {
	.write = ath6kl_acl_mac_del_write,
	.open = ath6kl_debugfs_open,
	.owner = THIS_MODULE,
	.llseek = default_llseek,
};
...
#ifdef SUPP_ACL_4_CUST
	debugfs_create_file("acl_policy", 0777, ar->debugfs_phy, ar,
			    &fops_acl_policy);

	debugfs_create_file("acl_mac_list", 0777, ar->debugfs_phy, ar,
			    &fops_acl_mac_list);

	debugfs_create_file("acl_mac_add", 0777, ar->debugfs_phy, ar,
			    &fops_acl_mac_add);

	debugfs_create_file("acl_mac_del", 0777, ar->debugfs_phy, ar,
			    &fops_acl_mac_del);
#endif

 ...
 */
int wmi_ap_set_acl_policy_cmd(struct wmi *wmi, u8 if_idx, u8 policy)
{
    ...
    switch(policy & (~(AP_ACL_RETAIN_LIST_MASK))){
    case AP_ACL_DISABLE:
    case AP_ACL_ALLOW_MAC:
    case AP_ACL_DENY_MAC:
        break;
    default:
		ath6kl_err("[%s]    invalid acl policy: %d\n", __FUNCTION__, policy);
		return -EINVAL;
	...
	ret = ath6kl_wmi_cmd_send(wmi, if_idx, skb, WMI_AP_ACL_POLICY_CMDID,
				  NO_SYNC_WMIFLAG);
	return ret;
 ...
 */
#if 0
int wmi_ap_acl_mac_list_cmd(struct wmi *wmi, u8 if_idx, struct wmi_ap_acl_mac_cmd *acl)
{
	...
	skb = ath6kl_wmi_get_new_buf(sizeof(struct wmi_ap_acl_mac_cmd));
	if (!skb)
		return -ENOMEM;

	cmd = (struct wmi_ap_acl_policy_cmd *) skb->data;
	memcpy(cmd,acl,sizeof(*acl));
    
	ret = ath6kl_wmi_cmd_send(wmi, if_idx, skb, WMI_AP_ACL_MAC_LIST_CMDID,
				  NO_SYNC_WMIFLAG);
	return ret;
    
}
#else
int wmi_ap_acl_mac_list_cmd(struct wmi *wmi, u8 if_idx, struct wmi_ap_acl_mac_cmd *mac_cmd)
{
	struct sk_buff *skb;
	int ret;
	...
	memcpy((char *)skb->data, (char *)mac_cmd, sizeof(struct wmi_ap_acl_mac_cmd));

	{
		...
		ptr = (char *)skb->data;
		printk("\n[%s] dump skb buffer b4 send it to wmi\n", __func__);

		for (i = 0; i < sizeof(struct wmi_ap_acl_mac_cmd); ++i)
			printk("%02x ", *ptr++);
	}

	printk("\n\n");

	ret = ath6kl_wmi_cmd_send(wmi, if_idx, skb, WMI_AP_ACL_MAC_LIST_CMDID, NO_SYNC_WMIFLAG);
